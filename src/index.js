...
export function jGraph(fileName, fileName2) {
  ...
  async function readDataFile(fileName2, fileName) {
    try {
      // Ensure d3.csv properly loads the data
      const jGraphData = await d3.csv(fileName2);
      console.log('Loaded data from ' + fileName2 + ' : ', jGraphData);
      if (!Array.isArray(jGraphData)) {
        throw new Error('Data loaded ' + fileName2 + ' is not an array.');
      }
  ...
  // sort arrayColumns by decreasing values of chemical shift
     ...
     for (i = 0; i < len; ++i) indicesSorted[indices[i]] = i;
      ...
      dataColumns.sort((a, b) => {
        return a.chemShift < b.chemShift
          ? 1
          : a.chemShift > b.chemShift
          ? -1
          : 0;
      });
     ...
     for (i = 0; i < unassignedCouplings.content.length; i++) {
      ...
      for (let i = 0; i < numberItem; i++) {
        ...
        const curPosRight =
          widthOfThePlot - (numberItem - i - 0.5) * smallSpace;
...
*/
   ...
   */
      ...
      function afterReadSpectrum(chemShift) {
        ...
        var brush = d3
          ...
          .on('end', function (event) {
            updateChart(event); // Pass the event object to the updateChart function
        ...
        function updateChart(event) {
          ...
          d3.select(this) // Use 'this' to refer to the element
            .select('.lineG')
            .transition()
            .duration(1000)
            .attr(
      ...
      async function loadSpectrum(fileName) {
        try {
          // Use d3.csv to load data and parse it in one step
          const spectrumData = await d3.csv(fileName, (d) => ({
            chemShift: +d.x, // Assuming 'x' is the column name for chemical shifts in your CSV
            value: +d.y, // Assuming 'y' is the column name for values in your CSV
          }));
          // Log to check if the data is loaded correctly
          return spectrumData;
        } catch (error) {
          console.error(
      ...
      const spectrumData = await loadSpectrum(fileName);
  ...
  readDataFile(fileName2, fileName);
}
